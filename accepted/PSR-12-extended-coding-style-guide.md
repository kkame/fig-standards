...
PSR-1의 'StudlyCaps'라는 용어는 파스칼 케이스(PascalCase)로 해석해야하며(MUST) 첫 단어가 첫 문자를 포함하여 대문자로 표시되어야합니다.
...
PHP만 존재하는 파일에서는 닫는 `?>`태그를 생략해야합니다 (MUST).
...
줄 끝 부분에 공백 문자가 없어야합니다 (MUST NOT).
...
클래스에 `use` import 문 다음에 아무것도 없는 경우, 클래스를 닫는 중괄호는 `use` import 문 다음에 있어야합니다 (MUST).
...
그렇지 않으면 `use` import 문 다음에 빈 줄이 있어야합니다(MUST).
...
`insteadof` 와 `as`연산자를 사용할 때는 들여 쓰기, 간격 및 새 줄을 다음과 같이 사용해야합니다.
...
프로젝트의 PHP 최소 버전이 상수의 가시성 (PHP 7.1 이상)을 지원하는 경우 모든 상수에 대한 가시성을 반드시(MUST) 선언해야합니다.
...
디폴트 값을 가진 메소드 및 함수 인수는 인수 목록의 끝에 와야합니다 (MUST).
...
조건들 사이의 부울-bool 연산자는 언제나 둘의 혼합이 아닌 줄의 시작 또는 끝 부분에 있어야합니다 (MUST).
...
조건들 사이의 부울 연산자는 언제나 둘의 혼합이 아닌 라인의 시작 또는 끝 부분에 있어야 합니다(MUST).
...
모든 이항 연산자와 삼항 연산자는 최소한 하나의 공백 앞에 와야합니다(MUST).

여러 개의 공백은 가독성 목적으로 사용할 수 있습니다(MAY). 

여기에는 [산술-arithmetic][], [비교-comparison][], [할당-assignment][], [비트 별-bitwise][], [논리-logical][] (단항 일 때 '!'제외), [문자열 연결-string concatenation][], [유형-type][] 연산자, 특성-trait 연산자 와 파이프 연산자(예 :`ExceptionType1 | ExceptionType2 $ e`) 등 이 모두 포함됩니다.

증가/감소 연산자와 증가/감소되는 변수 사이에 공백이 없어야합니다(MUST NOT).

다른 연산자는 정의되지 않은 채로 남겨둡니다.
...
타입 캐스팅 괄호 안에는 공백이 없어야합니다(MUST NOT).
